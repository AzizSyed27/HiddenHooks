"""Ingest OHN Waterbody polygons into the candidates table (Phase 2: FMZ 16 + 17)."""
...
from config import COMBINED_BBOX, DATABASE_URL, OHN_WATERBODY_DIR

UPSERT_SQL = text("""
    INSERT INTO candidates
        (ohn_id, source_dataset, candidate_type, name, fmz_zone, geom, area_m2)
    VALUES
        (:ohn_id, 'waterbody', 'polygon', :name, :fmz_zone,
...
def load_waterbodies(
    ...
    return gdf[["ohn_id", "name", "fmz_zone", "geometry", "area_m2"]]
...
def ingest(engine) -> None:
    ...
    gdf = load_waterbodies(OHN_WATERBODY_DIR, COMBINED_BBOX, regions)
