...
**Deliverable**: A web page that displays waterbody polygons within ~20km of your home, ranked by distance to nearest road, on a Mapbox map with a custom muted basemap. Click a polygon, see its rank and distance in a side panel.

**Total time**: 2-3 weekends alongside other commitments.
...
**Plan Mode prompt**: "Design a PostGIS schema for v1 of HiddenHooks. Each row is a candidate water body. Candidates can be either polygons (lakes/ponds) or linestring segments (stream reaches) — both stored in the same table with a `candidate_type` discriminator. Required columns: source OHN ID (preserve for re-joining), candidate type, name (nullable), geometry with explicit SRID, surface area (for polygons), length (for linestrings), and a `dist_to_road_meters` column for our v1 feature. Include a spatial GIST index. Use EPSG:3161 (Ontario MNR Lambert) for the geometry column. Show me the SQL before generating files."

Verify in the proposed schema:
- [ ] Geometry column has explicit SRID 3161
- [ ] Spatial GIST index exists on the geometry column
- [ ] `candidate_type` is an enum or constrained text, not free-form
- [ ] OHN ID is preserved for traceability back to source data
- [ ] Migrations live in `backend/db/migrations/` (or use a single SQL file for v1 — Alembic is overkill this early)

### Apply the schema

- [ ] Save the schema to `backend/db/schema.sql`
- [ ] Apply it via psql or a small Python script
- [ ] Verify: list tables, check the spatial index exists (`\di` in psql), confirm column types

---

## Part 3 — Data ingestion (2-3 hours)

You already have OHN data from Phase 0. This pulls a slice of it into your database.
...
- [ ] Copy your Phase 0 OHN shapefiles to `data/ohn/` in the project
- [ ] Confirm all supporting files are present (`.shp`, `.shx`, `.dbf`, `.prj`)

### Write the ingestion script

**Plan Mode prompt**: "Write a Python script `backend/ingest/ohn.py` that reads the OHN waterbody shapefile from `data/ohn/`, filters to features intersecting our TEST_BBOX (defined in config.py), reprojects to EPSG:3161, and inserts them into the candidates table. Use GeoPandas for reading and reprojection, SQLAlchemy for inserts. Handle both polygon and linestring features (waterbodies vs. stream reaches). Make it idempotent — running twice doesn't duplicate rows. Show me the plan before generating code."
...
- [ ] Polygon vs. linestring distinction handled correctly when populating `candidate_type`
- [ ] Idempotency strategy is real (e.g., upsert on OHN ID, not just insert)

After implementation:
- [ ] Run the script
- [ ] Connect to Postgres, run `SELECT COUNT(*), candidate_type FROM candidates GROUP BY candidate_type;`
- [ ] Should be tens to hundreds of features. If zero, bbox is wrong. If tens of thousands, bbox is too big.
...
- [ ] Add the candidates table as a layer
- [ ] Verify polygons render in the right place at the right scale
- [ ] If anything looks off, fix CRS handling before proceeding
...
**Plan Mode prompt**: "Write `backend/scoring/dist_to_road.py` that computes the distance from each candidate to the nearest road, in meters, and updates the `dist_to_road_meters` column. Use PostGIS `ST_Distance` with the spatial index. Recommend the most efficient query approach. Show the SQL before writing."
...
After running:
- [ ] Spot-check 3-5 candidates in QGIS — does each distance value match your eyeball estimate?
- [ ] `SELECT MIN(dist_to_road_meters), MAX(dist_to_road_meters), AVG(dist_to_road_meters), COUNT(*) FROM candidates;` — sanity check the distribution
...
- [ ] Pydantic models validate the response shape
...
**Plan Mode prompt**: "Create a Next.js page in `frontend/app/page.tsx` for the HiddenHooks map view. Layout: full-screen Mapbox map (using react-map-gl with the custom basemap from NEXT_PUBLIC_MAPBOX_STYLE), centered on Rouge Park. A collapsible left side panel (320px wide) using shadcn Sheet component, showing the ranked candidate list. On mount, fetch /candidates from `http://localhost:8000`. Render each candidate as a colored polygon overlay on the map: high-rank candidates brighter, low-rank candidates more muted. Click a polygon OR a list item, the side panel highlights that candidate and shows: name (or 'Unnamed [type]'), rank, distance to road, and a small breakdown card. Use Framer Motion for the side panel slide-in. Lucide icons for the chevron and any toggles. Show me the component structure and proposed file layout before generating."
...
- [ ] Polygon styling is computed from rank, not hardcoded
...
- [ ] Candidate polygons appear, color-coded by rank
- [ ] Side panel shows ranked list
- [ ] Click a candidate — both pin and list highlight
- [ ] Walk through the data path mentally: this polygon came from a shapefile → ingestion script → Postgres → FastAPI query → JSON response → React state → Mapbox layer. You should be able to explain each hop.
...
If both top and bottom rankings make geographic sense, **Phase 1 is functionally complete.**
...
- [ ] A live page shows ranked candidates in the test region
- [ ] Custom muted basemap is rendering, not default Mapbox
- [ ] Click-to-select works for both pins and list items
...
- Top 5 ranked candidates with rough coordinates (so we can compare against your Phase 0 calibration set when we have one)
...
Then we plan Phase 2: real ingestion of all FMZ 16 data, the four-component scoring formula, stream reach segmentation, and the species-value table.
